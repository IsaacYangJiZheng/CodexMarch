...
const OfflineOrderList = ({
  ...
  const handleCloseExportExcelDialog = () => {
    setOpenExportExcelDialog(false);
    setExcelFileName('');
  };

  const handleOpenExportExcelDialog = () => {
  ...
  React.useEffect(() => {
    if (ordersData?.length > 0) {
      setTableData(getData(ordersData));
    }
  }, [ordersData]);
    ...
    >
      ...
      <Box display='flex' alignItems='center' gap={2}>
        ...
        <Button
          ...
          onClick={handleOpenExportExcelDialog}
        >
  ...
  console.log('OrderList', ordersData);
  ...
  return (
    <Box>
      ...
      <Card sx={{mt: 2, p: 5}}>
        <Grid container spacing={2}>
          <Grid item xs={12} md={12}>
            <DataTable
              header={header}
              value={ordersData?.length > 0 ? ordersData : []}
              paginator
              rows={10}
              size='small'
              dataKey='_id'
      ...
      <Dialog
        ...
        onClose={handleCloseExportExcelDialog}
      >
        <DialogTitle>
          {formatMessage({id: 'finance.sales.export.dialog.title'})}
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            {formatMessage({id: 'finance.sales.export.dialog.description'})}
          </DialogContentText>
          <TextField
            autoFocus
            margin='dense'
            label={formatMessage({id: 'finance.sales.export.dialog.fileNameLabel'})}
            fullWidth
            variant='outlined'
            value={excelFileName}
            onChange={(e) => setExcelFileName(e.target.value)} // Update file name state
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseExportExcelDialog} color='warning'>
