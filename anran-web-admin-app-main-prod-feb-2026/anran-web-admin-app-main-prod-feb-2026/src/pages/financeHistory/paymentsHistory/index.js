...
const PaymentList = () => {
  ...
  const handleExportToExcel = () => {
    ...
    const fileName = `${formatMessage({id: 'finance.payments.export.fileName'})}${currentDateTime}`;
    const exportData = [];
    filteredData.forEach((data) => {
      exportData.push({
        [formatMessage({id: 'finance.payments.table.paymentDate'})]: dayjs(
          data.payDate,
        ).format('MM/DD/YYYY'),
        ...
        [formatMessage({id: 'finance.payments.table.paymentReference'})]:
          data.payReference,
        [formatMessage({id: 'finance.payments.table.status'})]: data.order.status,
      });
    });
            ...
            >
              ...
              <Grid size={{md: 3, xs: 12}}>
                <DatePicker
                  ...
                  sx={{width: '100%'}}
                  label={formatMessage({id: 'finance.payments.filter.dateFrom'})}
              ...
              <Grid size={{md: 3, xs: 12}}>
                <DatePicker
                  slotProps={{field: {clearable: true}}}
                  sx={{width: '100%'}}
                  label={formatMessage({id: 'finance.payments.filter.dateTo'})}
