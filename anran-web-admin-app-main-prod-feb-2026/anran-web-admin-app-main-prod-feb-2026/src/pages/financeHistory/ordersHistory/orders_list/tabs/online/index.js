...
const OnlineOrderList = ({setVisible, setSelectedRow, reCallAPI, loading, branchOptions}) => {
  ...
  const handleExportExcel = () => {
    ...
    const fileName = `OnlineSalesHistory${currentDateTime}`;
    const exportData = [];
    filteredData.forEach((data) => {
      exportData.push({
        ...
        Status: data.status,
        'Package Code': data.items[0]?.package?.packageCode || '',
        'Package Name': data.items[0]?.package?.packageName || '',
        Price: data.items[0]?.unitPrice || '',
        Quantity: data.items[0]?.quantity || '',
        'Net Amount': data.items[0]?.netAmount || '',
